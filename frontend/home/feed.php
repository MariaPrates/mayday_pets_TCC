<!--Feed-->
<div class="row row-posts" id="MarginFeedFullScreen">
    <?php 
    $query_postagens = "SELECT 	usuarios.nome, usuarios.contato, usuarios.foto, posts.* FROM usuarios  INNER JOIN posts ON usuarios.id = posts.id_usuario and posts.ativo = 1  ORDER BY posts.id DESC";
    $consulta = mysqli_query($conexao, $query_postagens);

    while($post = mysqli_fetch_object($consulta)):
        $query_interesse = 'SELECT * FROM interesse_usuarios_post WHERE id_post ='.$post->id;
        $consulta_interesse = mysqli_query($conexao_aux, $query_interesse);
        ...
        if($post->ativo == 1){
                ...
                $interesse= array();
                    if($post_usuario_interesse = mysqli_fetch_object($consulta_interesse)){
                        if($post_usuario_interesse->interesse == 1 AND $post_usuario_interesse->id_usuario == $user->id AND $post_usuario_interesse->id_post == $post->id ){
                                ...
                                <form action='' method='POST' class='display-mobile-responsive0 mt-1 float-end' id='form_interesse'>
                                    ...
                                    <input type='button' class='interesse_auxiliar btn ms-4 text-decoration-none ButtonRadius text-white BackgroundThemeinteressebtn' name='interesse_auxiliar' id='interesse_auxiliar' value='Interesse'>
                                </form>";
                                $interesse_aux[1] = "
                                <form action='' method='POST' class='display-mobile-responsive1 float-end' id='form_interesse'>
                                    <input type='hidden' id='InteresseButton' name='InteresseButton' value='0'>
                        ...
                        $interesse_aux = array();
                        $interesse_aux[0] = "
                        <form action='' method='POST' class='float-end display-mobile-responsive0 mt-1' id='form_interesse'>
                        ...
                        $interesse_aux[1] = "
                        <form action='' method='POST' class='float-end display-mobile-responsive1' id='form_interesse'>
                            <input type='hidden' id='InteresseButton' name='InteresseButton' value='2'>
                            <input type='hidden' id='acao' name='acao' value='interesse'>
                            <input type='hidden' id='grupo' name='grupo' value='postagens'>
                            <input type='hidden' id='id_post' name='id_post' value='$id_post_criptografia'>
